...
if (!window.manualLoad) {
  ...
  flights.forEach(f => {
    ...
    if (f.opensky && f.opensky.lat != null && f.opensky.lon != null) {
      ...
      m.bindPopup(`<b>${f.callsign}</b><br>${f.iata || ""}<br>Status: ${f.status}`);
    ...
    if (f.history && f.history.length > 1) {
      const latlngs = f.history.map(p => [p.lat, p.lon]);
      const pl = L.polyline(latlngs, { color: "#0b61ff", weight: 3, opacity: 0.6 }).addTo(polylineLayer);
      polylines[f.callsign] = pl;
    }
  });
...
function focusOnFlight(callsign) {
  ...
  fetch(`/api/flights?callsigns=${encodeURIComponent(callsign)}`)
    ...
    .then(data => {
      ...
      if (polylines[callsign]) {
        polylineLayer.removeLayer(polylines[callsign]);
        delete polylines[callsign];
      }
      ...
      if (f.history && f.history.length > 1) {
        ...
        polylines[callsign] = pl;

        let i = 0;
        const step = () => {
          if (i >= latlngs.length) return;
          map.panTo(latlngs[i], { animate: true, duration: 0.6 });
          i++;
          setTimeout(step, 400);
        };
        step();
      } else {
        alert(`No trajectory history for ${callsign}`);
      }
    })
